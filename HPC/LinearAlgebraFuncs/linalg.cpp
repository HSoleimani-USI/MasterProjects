...
double ss_dot(Field const& x, Field const& y)
{
    ...
    for (int i = 0; i < N; i++)
        ...
        MPI_Allreduce(&result, &resultOverAllRanks, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);    

    return resultOverAllRanks;
}
...
double ss_norm2(Field const& x)
{
    ...
    for (int i = 0; i < N; i++)
        ...
        MPI_Allreduce(&result, &normOverAllRanks, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);

    return sqrt(normOverAllRanks);
}

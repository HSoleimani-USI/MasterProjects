...
//
// Goal:
//    The goal of this assignment is to parallelize the prime number generator.
//    The program takes two inputs: size of input (N) and number of processors
//    (P). N denotes the number till which we should find all the primes and P
//    denotes the number of processors the program can use to finish the job.
// ============================================================================
...
void Primes(unsigned N) {
  ...
  for (iter = 0; iter < NUM_ITER; ++iter)
  {
    ...
    t_serial = walltime(time_start);
    printf("time for Serial  primes is : %lf\n",t_serial);
...
void ParallelPrimes(unsigned N, unsigned P) {
 ...
 // double time_start = walltime(0);
  ...
  for (iter = 0; iter < NUM_ITER; ++iter)
  {
    ...
    #pragma omp parallel for shared(N, flags) private(i,prime,div1,div2) lastprivate(lastPrime) reduction(+:count)
    for (i = 0; i < (N-1)/2; ++i) {    /* For every odd number */
      prime = 2*i + 3;
    ...
    // parallel_t = walltime(time_start);    
     printf("time for parallel prime is: %lf\n", wtime);
